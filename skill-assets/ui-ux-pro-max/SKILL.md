...
- **Stack**: <stack_name> (e.g. react, nextjs, vue, svelte, astro, html-tailwind, react-native, flutter, swiftui)
...
| React perf / info | `react` | `--domain react "rerender memo list"` |
...
| Stack | Focus |
|-------|-------|
| `html-tailwind` | CSS, layout, spacing, components |
| `react` | Components, hooks, state, performance |
| `nextjs` | SSR, routing, optimization, layouts |
| `astro` | Static generation, performance, islands |
| `vue` | Components, reactive state, layouts |
| `nuxtjs` / `nuxt-ui` | SSR, components, layouts |
| `svelte` | Components, stores, transitions |
| `swiftui` | iOS layouts, views, navigation, lists |
| `react-native` | Cross-platform components, lists, performance |
| `flutter` | Widgets, layouts, state management |
| `shadcn` | Component structure, clean markup, theming |
| `jetpack-compose` | Android layouts, state, lists |
...
- Stack: html-tailwind (or any other project tech stack)
...
```bash
python3 skills/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack <stack_name>
```
...
- Always add `--stack <stack_name>` for implementation-specific guidance
